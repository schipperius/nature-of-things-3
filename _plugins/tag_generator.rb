...
module Jekyll
  ...
  class TagIndex < Page
    ...
    def initialize(site, base, tag_dir, tag)
      ...
      title_prefix             = site.config['tag_title_prefix'] || 'Related Articles: '
      self.data['title']       = "#{title_prefix}#{tag}"
      # Set the meta-description for this page.
      meta_description_prefix  = site.config['tag_meta_description_prefix'] || 'Related Articles: '
  ...
  class TagFeed < Page
    ...
    def initialize(site, base, tag_dir, tag)
      ...
      self.process(@name)
      # Read the YAML data from the layout page.
      self.read_yaml(File.join(base, '_includes'), 'aside', 'tag_feed.xml')
      self.data['tag']    = tag
      # Set the title for this page.
      title_prefix             = site.config['tag_title_prefix'] || 'Related Articles: '
      self.data['title']       = "#{title_prefix}#{tag}"
